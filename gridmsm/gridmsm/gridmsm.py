...
class GridMarkovStateModel(object):
    ...
    ----------
    """
    def __init__(self, n_bins, min, max, prior=1.0):
        self.n_bins = n_bins
        self.min = min
        self.max = max
        self.prior = prior
    ...
    def _discretize(self, datasequence):
        ...
        if np.any(np.logical_or(datasequence < self.min, datasequence > self.max)):
            raise ValueError('Data out of min/max bounds')
                
        #labels = np.zeros(len(datasequence), dtype=np.int)
        binassign = np.zeros((self.n_features, len(datasequence)), dtype=np.int)
        for i in range(self.n_features):
            binassign[i] = np.digitize(datasequence[:, i], self.grid) - 1
        labels = np.dot(self.n_features**np.arange(self.n_features), binassign)
        assert np.max(labels) < self.n_states
        return labels

    def _countsmat(self, X):
    ...
    def fit(self, X, method='mle'):
        ...
        method : {'mle', 'map'}
            ...
            transition matrix. If 'mle', the `prior` field is ignored.
        ...
        if method == 'mle':
            ...
            self.transmat_ = np.asarray(msmlib.estimate_transition_matrix(rc).todense())
        else:
    ...
    def logevidence(self, X, terms='all'):
        ...
        for trajectory in X:
            ...
            logevidence_transmat += _log_transmat_evidence(
                countsmat, starting_state, self.prior)
