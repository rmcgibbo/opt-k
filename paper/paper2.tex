...
Introduction to MSMs via the propagator theory motivates the view of the parameterization problem as one of \emph{function estimation}. This perspective has a critical disadvantage: the natural error metric -- the difference between the estimated MSM and true propagator is intrinsically uncomputable. Instead, a compelling alternative is to view the problem as one of time series \emph{prediction}. We attempt to build models which predict new data. Here, the quality of the prediction (as measured for example by the likelihood that the model would generate an observed sample) is a natural measure of the quality of the model.

%The basic framework for model selection intimated in the section involves minimization of an error metric characteristics of the model -- for example the norm of the between the model's eigenvalues and the propagator's true eigenvalues, or the L2 norm of the eigenvectors ( I think frank worked with this in one paper). Unfortunately, these quantities are not computable from a given model $M = \{S, T\}$ and data set $D$. The only access we have to the true propagator is via sampling new data. Therefore, the only way to know that we have constructed a model that is consistent with the propagator is to test how well the model would predict the new data. 

%The only available access to the information in the propgator is our sample $D$. For this reason, we instead emphasize the ability of the model to predict $D$.

%On one hand, $M$ can be viewed as being fit _on_ $D$, but from another perspective $M$ is something that can _generate_ $D$. We thus can consider how predictive $M$ is.
